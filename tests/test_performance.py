...
def test_compute_monthly_returns_primeiro_mes_sem_mes_anterior():
    """No primeiro mês da série, patrimônio/investido de "início de mês"
    são 0 (não existia carteira antes) — o retorno do mês coincide com o
    retorno nominal cumulativo simples (wealth/invested - 1)."""
    daily_points = [
        (date(2026, 8, 4), 1000.0, 1000.0),
        (date(2026, 8, 31), 1050.0, 1000.0),  # só a última data do mês conta
    ]
    result = compute_monthly_returns(daily_points, {})
    assert len(result) == 1
    r = result[0]
    assert r.year == 2026 and r.month == 8
    assert abs(r.nominal - 0.05) < 1e-9  # 1050/1000 - 1
    assert r.real is None  # sem IPCA de agosto informado
...
def test_compute_monthly_returns_ipca_do_mes_nao_publicado_e_none():
    daily_points = [(date(2026, 9, 1), 1000.0, 1000.0)]
    result = compute_monthly_returns(daily_points, {})
