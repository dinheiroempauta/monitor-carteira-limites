...
<style>
  ...
  /* Tudo cabe numa tela só, sem scroll em nenhuma direção (a 100% de
     ...
     viewport, e a única área que "respira" de verdade — o dash-grid —
     recebe o espaço que sobra (flex: 1) depois de masthead, cabeçalho,
     KPIs e rodapé, todos compactados ao essencial. min-height: 0 em
     ...
     sentido (3 gráficos empilhados não cabem em altura nenhuma) — nesse
  ...
  main {{
    ...
    padding: 14px 24px 10px; flex: 1 1 auto; display: flex; flex-direction: column; min-height: 0;
  }}
  .eyebrow {{ margin: 0; }}
  .page-heading {{ margin: 2px 0; font-size: 1.7rem; }}
  .page-subheading {{ margin: 0 0 10px; font-size: 13.5px; }}

  .kpi-row {{ flex: 0 0 auto; display: grid; grid-template-columns: repeat(auto-fit, minmax(190px, 1fr)); gap: 10px; margin: 0 0 12px; }}
  .kpi-tile {{ border: 1px solid var(--line-strong); background: var(--paper-raised); border-radius: 3px; padding: 10px 16px; }}
  .kpi-tile .kpi-label {{ font-family: var(--font-mono); font-size: 10.5px; letter-spacing: .08em; text-transform: uppercase; color: var(--muted); margin-bottom: 4px; }}
  .kpi-tile .kpi-value {{ font-family: var(--font-mono); font-size: 1.3rem; font-variant-numeric: tabular-nums; }}
  ...
  /* Bento: composição ocupa a coluna esquerda inteira (2 linhas);
     ...
     assim as três áreas usam a largura e a altura disponíveis de forma
     proporcional ao que cada uma precisa mostrar, em vez de três
     colunas iguais com muito espaço vazio. */
  .dash-grid {{ flex: 1 1 auto; min-height: 0; display: grid; grid-template-columns: 1.1fr 1.6fr; grid-template-rows: 1fr 1fr; gap: 14px; }}
  .dash-grid .chart-card {{ margin: 0; min-height: 0; display: flex; flex-direction: column; padding: 12px 16px 10px; }}
  ...
  @media (max-width: 900px) {{
    ...
    .dash-grid {{ display: grid; grid-template-columns: 1fr; grid-template-rows: none; }}
    .chart-card.tall, .chart-card.wide-top, .chart-card.wide-bottom {{ grid-column: auto; grid-row: auto; }}
    .dash-grid .chart-card {{ margin: 0 0 14px; }}
    .chart-card.tall .chart-svg-wrap, .chart-card.wide-top .chart-svg-wrap, .chart-card.wide-bottom .chart-svg-wrap {{ height: 320px; flex: none; }}
  }}
  .chart-svg-wrap {{ position: relative; flex: 1 1 auto; min-height: 0; }}
  .chart-svg-wrap canvas {{ max-width: 100%; }}

  footer {{ flex: 0 0 auto; padding: 6px 24px !important; font-size: 12px !important; line-height: 1.4 !important; }}
